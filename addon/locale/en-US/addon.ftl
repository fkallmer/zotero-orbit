...
graph-axes-note = Citations (vertical) against publication year. Mark size is how many works each one cites. Arrows point from the citing work to the cited one. Scroll to zoom, drag to pan.
graph-role-seed = This work
graph-role-reference = Cited by it
graph-role-citing = Citing it
graph-no-years = None of the surrounding works carry a publication year, so there is nothing to place on a time axis.
graph-dropped-no-year = { $count } without a year, not shown
